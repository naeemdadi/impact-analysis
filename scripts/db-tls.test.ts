...
test("production requires a TLS-enabled DATABASE_URL while dev accepts plaintext", () => {
  ...
  assert.equal(databaseUrlHasTls("postgresql://u:p@h/db?sslmode=disable"), false);
  ...
  assert.throws(() => assertDatabaseTls("postgresql://u:p@h/db?sslmode=disable", "production"), /must enable TLS/);
